...
int isAsciiDigit(int x) {
  return (!((x + negate(0x30)) >> 31)) & ((x + negate(0x3A)) >> 31);
}//x - 0x30 >=0 & x - 0x3A < 0
/* 
...
int howManyBits(int x) {
  ...
  complement = complement >> bit8;
  int bit4 = !!(complement >> 4) << 2;
  complement = complement >> bit4;
  int bit2 = !!(complement >> 2) << 1;
  complement = complement >> bit2;
  int bit1 = !!(complement >> 1) << 0;
  complement = complement >> bit1;
  int bit0 = !!(complement);
